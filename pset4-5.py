# -*- coding: utf-8 -*-
"""
Created on Tue Aug  1 12:55:53 2017
...
WORDLIST_FILENAME = "wordsforpset4.txt"
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    countforloop = len(hand)
    while (countforloop > 0):
        print("loop value", countforloop)
        # Display the hand
        print("Current Hand: ", end = "")
        displayHand(hand)
    
        # Ask user for input
        guess = input("Enter word, or a \".\" to indicate that"\
                      + " you are finished: ")
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                score = score + getWordScore(guess, n)
                print("\"" + guess + "\"", "earned", getWordScore(guess, n), "points.", "Total:", score, "points")
                
                ## Update the hand
                
                #count of dictionary items in hand before whose value is set to 0
                countX = 0
                for char in hand.values():
                    if (char == 0):
                        countX += 1
                print ("items set to zero before", countX)
                ...
                #count of dictionary items in hand after updated hand
                countY = 0
                for char in hand.values():
                    if (char == 0):
                        countY += 1
                
                print("Items set to zero after", countY)
                #updating loop variable
                countforloop = countforloop - (countY - countX)
                print(countforloop)
...
def playGame(wordList):
    ...
    #Flag to determine in its the first game
    flagforfirstgame = True
    
    while(userinput != "e"):
        userinput = input("Enter n to deal a new hand,"\
                          + " r to replay the last hand, "\
                          + "or e to end game: ")
        
        if (flagforfirstgame == True and userinput == "r"):
            print("You have not played a hand yet. Please play a new hand first!")
            continue
        elif (flagforfirstgame == False and userinput == "r"):
            print(previoushand)
            playHand(previoushand, wordList, HAND_SIZE)
        
        if (userinput == "n"):
            flagforfirstgame = False
            hand = dealHand(HAND_SIZE)
            
            
            #making copy so that it can be refrenced back
            previoushand = hand.copy()
            print(previoushand)
            
            playHand(hand, wordList, HAND_SIZE)
        
        if (userinput != "e" and userinput != "r" and userinput != "n"):
